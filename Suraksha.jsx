...
//  SOS MODULE
// ══════════════════════════════════════════════════════════════
function SOSModule({ contacts, onClose, addToast }) {
  ...
  const startSOS = () => {
    ...
    timerRef.current = setInterval(() => {
      setCount(c => {
        if (c <= 1) { clearInterval(timerRef.current); setState("sent"); addToast("🚨 SOS Sent!", `${contacts.length||"Your"} emergency contact${contacts.length!==1?"s":""} notified with GPS location.`,"success"); setTimeout(()=>setState("idle"),7000); return 0; }
        return c - 1;
      });
    }, 1000);
  };
  ...
  return (
    <div className="mp anim">
      ...
      {state==="sent"&&<div className="sos-ok"><div style={{fontSize:".82rem",color:"#86efac",fontWeight:600}}>✅ SOS Alert Sent Successfully</div><div style={{fontSize:".7rem",color:"rgba(255,255,255,.43)",marginTop:4}}>{contacts.length>0?`${contacts.length} contact${contacts.length!==1?"s":""}` :"Your emergency contacts"} notified with live GPS location.</div></div>}
      ...
      <div className="sos-actions">
        {[["📱","Call 112","Emergency services"],["💬","WhatsApp","Alert contacts"],["📧","Email Alert","Full incident report"],["📍","Share Location","Google Maps link"]].map(([icon,label,sub])=>(
          <button key={label} className="sos-act" onClick={()=>addToast(`${icon} ${label}`,"Auto-triggered on SOS in production via Twilio + Firebase Functions.","info")}><span style={{fontSize:"1.25rem"}}>{icon}</span><div><div>{label}</div><div style={{fontSize:".62rem",color:"rgba(255,255,255,.35)"}}>{sub}</div></div></button>
        ))}
      </div>
      <div className="divider"/>
      <p className="hint">5-second countdown lets you cancel. Contacts receive GPS location link via SMS + WhatsApp in production (Twilio).</p>
    </div>
  );
}
  ...
  return (
    <div className="mp anim">
      ...
      {route&&<>
        ...
        <button className="share-btn" style={{marginTop:".75rem"}} onClick={()=>addToast("📤 Route Shared","Safe route sent to all emergency contacts.","success")}>📤 Share Route with Contacts</button>
        <button className="share-btn" onClick={()=>addToast("🗺️ Opening Maps","Requires VITE_MAPS_API_KEY in .env for production.","info")}>🗺️ Open in Google Maps</button>
      </>}
...
function CommunityModule({ onClose, addToast }) {
  ...
  return (
    <div className="mp anim">
      ...
      {tab==="map"&&<>
        <div style={{background:"rgba(124,58,237,.08)",border:"1px solid rgba(124,58,237,.2)",borderRadius:13,padding:"2rem",textAlign:"center",margin:".5rem 0"}}><div style={{fontSize:"2.5rem",marginBottom:".6rem"}}>🗺️</div><div style={{fontSize:".88rem",fontWeight:600,marginBottom:".4rem"}}>Safety Heat Map</div><div style={{fontSize:".74rem",color:"rgba(255,255,255,.4)",lineHeight:1.65,marginBottom:"1rem"}}>Interactive Google Maps heat map showing high-risk and safe zones based on community reports.</div><button className="share-btn" style={{display:"inline-flex",width:"auto",padding:"9px 18px"}} onClick={()=>addToast("🗺️ Maps","Add VITE_MAPS_API_KEY to .env to enable Google Maps integration.","info")}>Enable Google Maps →</button></div>
    ...
    <div className="dash">
        ...
        <div className="helplines">
          {HELPLINES.map(h => (
            <div key={h.number} className="hl-row"
              onClick={() => addToast(`📞 ${h.label}`, `Dial ${h.number} — in production this opens the phone dialer directly.`, "info")}>
      ...
      {module && (
        <div className="mp-wrap" onClick={e => e.target === e.currentTarget && closeModule()}>
          {module === "sos"       && <SOSModule       contacts={contacts} onClose={closeModule} addToast={addToast} />}
